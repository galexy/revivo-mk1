...
Purpose: Establish the persistence layer schema for users, households, and refresh tokens. Add household scoping to all user-owned data.
...
<task type="auto">
  <name>Task 1: Add HouseholdIdType to ORM types</name>
...
Add import for HouseholdId at top of file. Export HouseholdIdType.
  ...
  <verify>
Run `python -c "from src.adapters.persistence.orm.types import HouseholdIdType; print(HouseholdIdType)"` to verify import works.
  </verify>
  <done>HouseholdIdType converts between HouseholdId and String for database storage.</done>
</task>

<task type="auto">
  <name>Task 2: Update tables.py with auth tables and household columns</name>
...
```python
# Households table (owns all user data - accounts, transactions, etc.)
households = Table(
    "households",
    metadata,
    Column("id", HouseholdIdType(36), primary_key=True),  # hh_xxx
    Column("name", String(255), nullable=False),
    Column("owner_id", UserIdType(36), nullable=False),  # FK added after users table created
...
```python
# Users table (expanded for authentication)
users = Table(
    "users",
    metadata,
    ...
    Column("password_hash", String(255), nullable=False),  # Argon2 hash
...
refresh_tokens = Table(
    ...
    Column("token_hash", String(64), nullable=False),  # SHA-256 hash of token
...
Add import for HouseholdIdType at top.
  ...
  <verify>
Run `python -c "from src.adapters.persistence.orm.tables import households, users, refresh_tokens; print('Tables defined')"` to verify table definitions compile.
  </verify>
  <done>Table definitions include households, expanded users, refresh_tokens, and household_id on existing tables.</done>
</task>

<task type="auto">
  <name>Task 3: Create Alembic migration 006</name>
...
def upgrade() -> None:
    ...
    # Create default household (use a fixed ID for reproducibility in tests)
    ...
    # Update existing records to use default household
    ...
    # Update existing users with default household and placeholder password
    conn.execute(
        sa.text(
...
```
  ...
  <verify>
Run `cd /workspace && alembic upgrade head` to apply migration. Then verify with:
