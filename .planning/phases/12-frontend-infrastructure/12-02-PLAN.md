...
<task type="auto">
  ...
  <action>
    ...
    8. Create `apps/web/src/vite-env.d.ts`:
       ...
       ```
    9. Update `apps/web/project.json` — keep targets empty, @nx/vite plugin infers build/serve/preview/test:
       ```json
       {
         "name": "web",
  ...
  <verify>
    ...
    - **Visual (Chrome):** Use Claude Code Chrome to open http://localhost:5173 and take a screenshot. Confirm the page renders with the "Personal Finance" heading and no blank/error screen.
  </verify>
