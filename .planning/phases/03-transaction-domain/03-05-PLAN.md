...
must_haves:
  truths:
    ...
    - "TransactionService can update transaction splits and amount"
  ...
  artifacts:
    - path: "src/application/services/transaction_service.py"
      provides: "TransactionService with full CRUD, split updates, and validation"
  ...
  key_links:
    ...
    - from: "src/application/services/transaction_service.py"
      ...
      via: "Transaction.create factory and update_splits"
      pattern: "Transaction.create|update_splits"
---
...
<task type="auto">
  ...
  <action>
Create TransactionService with full CRUD, mirror transaction creation, split updates, and validation:
...
class TransactionService:
    ...
    def _sync_mirrors_for_split_update(
        ...
        existing_mirrors = self._uow.transactions.get_mirrors_for_source(source_transaction.id)
    ...
    def update_transaction_splits(
        ...
        with self._uow:
            ...
            if not new_splits:
                return TransactionError("NO_SPLITS", "Transaction must have at least one split")
            valid, msg = self._validate_splits(user_id, txn.account_id, new_splits, new_amount)
            if not valid:
                return TransactionError("INVALID_SPLITS", msg)
...
```
  ...
  <done>
TransactionService created with full CRUD, auto-payee creation, mirror transaction management, split updates, and proper validation. Uses UnitOfWork for transaction boundaries.
  </done>
</task>
...
7. TransactionService can update splits and amount via update_transaction_splits()
8. TransactionService syncs mirror transactions when splits change
9. TransactionService propagates edits to mirrors per CONTEXT rules
10. Both services use UnitOfWork pattern
11. Both services emit domain events properly
</success_criteria>
