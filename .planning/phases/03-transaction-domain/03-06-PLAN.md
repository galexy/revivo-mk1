...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/adapters/api/schemas/transaction.py"
      provides: "Transaction Pydantic schemas"
      contains: "class CreateTransactionRequest"
...
```
  ...
  <done>
Pydantic schemas created for categories and transactions with proper validation, field constraints, and model configuration.
  </done>
</task>
...
```
  ...
  <done>
API routes created for categories (CRUD, tree structure) and transactions (CRUD, search, filter, status updates) with proper error handling.
  </done>
</task>
...
7. POST /api/v1/transactions/{id}/clear marks cleared
8. POST /api/v1/transactions/{id}/reconcile marks reconciled
9. DELETE /api/v1/transactions/{id} deletes transaction
10. API returns proper error responses with codes
</success_criteria>
