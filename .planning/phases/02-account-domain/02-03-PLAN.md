...
must_haves:
  truths:
    ...
    - "Money reconstructs from *_amount/*_currency columns (opening_balance, credit_limit)"
    - "InstitutionDetails reconstructs from institution_* columns"
    - "RewardsBalance reconstructs from rewards_value/rewards_unit columns"
    - "AccountId and UserId reconstruct from string columns"
    - "Enum types (AccountType, AccountStatus, AccountSubtype) reconstruct from string columns"
...
class SqlAlchemyAccountRepository:
    ...
    def _reconstruct_value_objects(self, account: Account) -> None:
        ...
        # Money - opening_balance (required field, always present)
        if hasattr(account, "opening_balance_amount") and account.opening_balance_amount is not None:
            object.__setattr__(
                account,
                "opening_balance",
                Money(Decimal(str(account.opening_balance_amount)), account.opening_balance_currency),
            )

        # Money - credit_limit (optional, for credit cards)
        if hasattr(account, "credit_limit_amount") and account.credit_limit_amount is not None:
            object.__setattr__(
                account,
                "credit_limit",
                Money(Decimal(str(account.credit_limit_amount)), account.credit_limit_currency),
            )

        # InstitutionDetails (optional)
        if hasattr(account, "institution_name") and account.institution_name is not None:
            object.__setattr__(
                account,
                "institution",
                InstitutionDetails(
        ...
        # RewardsBalance (optional, for rewards accounts)
        if hasattr(account, "rewards_value") and account.rewards_value is not None:
            object.__setattr__(
                account,
                "rewards_balance",
                RewardsBalance(Decimal(str(account.rewards_value)), account.rewards_unit),
            )
...
Imports work correctly.
  ...
  <done>
SqlAlchemyAccountRepository implemented with complete value object reconstruction for Money, InstitutionDetails, RewardsBalance, AccountId, UserId, and enum types. UnitOfWork updated with accounts property.
  </done>
</task>
...
- [ ] Value object reconstruction works: AccountId, UserId, enums, Money, InstitutionDetails, RewardsBalance
