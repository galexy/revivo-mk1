...
Ensure the mappers are started in lifespan (should already be there from Phase 1).
  </action>
  <verify>
Run:
```bash
...
curl http://localhost:8000/docs 2>/dev/null | grep -q "accounts" && echo "Accounts routes in docs" || echo "Check API manually"

# Or just verify imports
python -c "from src.adapters.api.app import app; print('App imports with accounts routes')"
```
  </verify>
  <done>
Account routes registered in FastAPI app. Endpoints visible at /docs.
  </done>
</task>
...
print(f'Account routes: {account_routes}')
assert len(account_routes) >= 10, 'Missing account routes'
print('All account routes registered')
"
```
</verification>
