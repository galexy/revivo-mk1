...
2. On success: navigate to '/dashboard' via `useNavigate()` from react-router-dom
...
Link to register: `<Link to="/register">Don't have an account? Sign up</Link>` below the form.
...
Link to login: `<Link to="/login">Already have an account? Log in</Link>` below the form.
...
If user is already authenticated, redirect to /dashboard (use useAuth + Navigate).
...
Replace the placeholder LoginPage and RegisterPage with imports from the actual page files:
```typescript
import { LoginPage } from '../pages/LoginPage';
import { RegisterPage } from '../pages/RegisterPage';
```
Remove the inline placeholder functions for LoginPage and RegisterPage. Keep the other placeholders (VerifyEmailPage, DashboardPage) for now.
