...
```bash
cd /workspace && pnpm add --filter @workspace/web react-hook-form zod @hookform/resolvers @radix-ui/react-icons axios @tanstack/react-router
```
Note: @tanstack/react-router ships its own TypeScript types — no separate @types package needed. TanStack Router is used directly (not deferred to Phase 14) to avoid throwaway react-router-dom dependency. Phase 14 will focus on TanStack Query + advanced routing patterns (loaders, search params, data prefetching).
