...
must_haves:
  ...
  artifacts:
    - path: "alembic/versions/005_add_split_identity.py"
      provides: "Database migration for split_id and source_split_id columns"
      contains: "split_id"
...
<task type="auto">
  ...
  <action>
1. Create migration file alembic/versions/005_add_split_identity.py:
...
Check tables: `cd /workspace && python -c "from src.adapters.persistence.orm.tables import split_lines, transactions; print('Tables loaded OK')"`
  </verify>
  <done>
- Migration 005 exists and runs successfully
...
2. Update transaction.py schemas:
...
3. Add ValueError exception handler to app.py:
...
4. Update TransactionService._sync_mirrors_for_split_update() in transaction_service.py:
...
<success_criteria>
- Migration 005 runs without errors (including backfill)
