...
must_haves:
  truths:
    ...
    - "Category type can be set and retrieved"
...
<objective>
Add comprehensive tests for split identity, validation error handling, and category types. Verify all Phase 3.1 success criteria are met.

Purpose: Ensure all UAT issues from Phase 3 are resolved and the new functionality works correctly.

Output: New and updated test files covering split IDs, validation, and category types.
</objective>
...
@.planning/phases/03.1-split-identity-validation-fixes/03.1-02-SUMMARY.md
@tests/integration/test_transaction_api.py
@tests/integration/test_category_api.py
</context>
...
<task type="auto">
  ...
  <action>
1. Add tests to test_transaction_api.py for validation error handling:
...
2. Add tests for split ID persistence and PATCH:
...
3. Add tests to test_category_api.py for category_type:
...
class TestCategoryType:
    ...
    def test_create_category_invalid_type_returns_400(
        self, client: TestClient, setup_user: UserId
    ) -> None:
        """Invalid category type should return 400."""
        response = client.post(
            "/api/v1/categories",
...
- Integration tests for category type pass
...
- [ ] Invalid account/category IDs return 400 (not 500)
- [ ] Split must have exactly one of category_id or transfer_account_id (or neither for uncategorized)
...
- Phase 3.1 success criteria verified by tests:
  ...
  5. Category type works
</success_criteria>

<output>
After completion, create `.planning/phases/03.1-split-identity-validation-fixes/03.1-03-SUMMARY.md`
</output>
