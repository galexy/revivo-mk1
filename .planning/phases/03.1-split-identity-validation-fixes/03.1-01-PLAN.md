...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/domain/model/category.py"
      provides: "Category with CategoryType enum"
...
<objective>
Add SplitId to entity identifiers, transform SplitLine from value object to entity with identity, add source_split_id to Transaction for mirror linkage, and add CategoryType enum to Category.
...
Output: Updated domain model files with SplitId, SplitLine entity, Transaction.source_split_id, and CategoryType enum.
</objective>
...
<task type="auto">
  <name>Task 1: Add SplitId and CategoryType to domain model</name>
  <files>
    src/domain/model/entity_id.py
    src/domain/model/category.py
    tests/unit/domain/test_entity_id.py
...
2. Add CategoryType enum to category.py BEFORE the Category class:
```python
...
3. Add category_type field to Category class with default EXPENSE:
```python
category_type: CategoryType = CategoryType.EXPENSE
```

4. Update Category.create() factory to accept optional category_type parameter (default EXPENSE)

5. Update Category.create_system_category() to accept optional category_type parameter (default EXPENSE)

6. Add test for SplitId in test_entity_id.py following existing test patterns:
...
2. Import check: `python -c "from src.domain.model.entity_id import SplitId; from src.domain.model.split_line import SplitLine; from src.domain.model.category import CategoryType; print('All imports OK')"`
...
5. CategoryType: `python -c "from src.domain.model.category import CategoryType; print(CategoryType.INCOME, CategoryType.EXPENSE)"`
</verification>
