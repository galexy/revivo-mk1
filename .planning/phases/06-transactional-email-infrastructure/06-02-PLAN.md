...
must_haves:
  truths:
    ...
    - "MJML templates compile to email-client-compatible HTML"
    - "Compiled HTML templates contain Jinja2 variables for runtime rendering"
  artifacts:
    ...
    - path: "src/adapters/email/templates/verification.mjml"
      provides: "MJML source for verification email"
      contains: "verification_link"
    - path: "src/adapters/email/templates/verification.html"
      provides: "Compiled HTML template (table-based, email-client compatible)"
      contains: "verification_link"
  key_links:
...
<objective>
Create the email service domain port and SMTP adapter implementation with MJML templates.

Purpose: Establish clean architecture email infrastructure with email-client-compatible HTML templates that compile from MJML source.
Output: EmailService protocol, SmtpEmailAdapter, MJML source files, and compiled HTML templates.
</objective>
...
<task type="auto">
  <name>Task 2: Create SMTP adapter with MJML templates</name>
  <files>
    src/adapters/email/__init__.py
    src/adapters/email/smtp_adapter.py
    src/adapters/email/templates/base.mjml
  ...
  <action>
Create the email adapter package with SMTP implementation and MJML templates:
...
2. Create `src/adapters/email/smtp_adapter.py`:
   ...
   - `is_available()` method with lazy check
...
3. Create Jinja2 template environment:
   ...
   - Load the compiled .html file (not .mjml) and .txt file

4. Create MJML source templates (these are the source of truth, edited by humans):
   - `templates/base.mjml`: MJML base with mj-body, mj-section, mj-column structure
   - `templates/verification.mjml`: Verification email with:
     - mj-text for greeting and instructions
     - mj-button for verify CTA with {{ verification_link }}
     - mj-text with raw link for copy/paste
     - mj-text stating "This link expires in {{ expiry_hours }} hours"
     - Footer with {{ company_name }} and {{ support_email }}

5. Compile MJML to HTML:
   - Run `make build-emails` (created in Plan 06-01)
   - This produces verification.html with table-based layout
   - Jinja2 variables {{ verification_link }}, {{ expiry_hours }}, etc. are preserved in output

6. Create plain text template manually:
   - `templates/verification.txt`: Plain text version with same variables
   - No MJML compilation needed for .txt files

7. Create `get_email_service()` factory:
...
1. Edit .mjml files (human-readable, clean markup)
2. Run `make build-emails` to compile to .html
3. Commit both .mjml source and .html output
4. At runtime, Jinja2 renders the .html with actual values
  </action>
  <verify>
...
Verify MJML source exists: `ls src/adapters/email/templates/*.mjml`
Verify compiled HTML exists: `ls src/adapters/email/templates/*.html`
Verify HTML contains Jinja2 variables: `grep "verification_link" src/adapters/email/templates/verification.html`
Run `pytest tests/ -x -q` passes (no regressions).
  </verify>
  <done>
SmtpEmailAdapter implements EmailService protocol. MJML templates compile to email-client-compatible HTML with preserved Jinja2 variables. get_email_service() factory returns configured adapter.
  </done>
</task>
...
3. MJML source files exist and are human-readable
4. Compiled HTML uses table-based layout (email-client compatible)
5. Jinja2 variables preserved in compiled HTML
6. No domain layer imports infrastructure (lint-imports clean)
...
- MJML source files in templates/ directory
- Compiled HTML templates are table-based (email-client compatible)
- Jinja2 variables ({{ verification_link }}, etc.) preserved after MJML compilation
